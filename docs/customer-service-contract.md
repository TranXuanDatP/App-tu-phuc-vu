...
> Hợp đồng resolve/verify/profile đã được gộp vào `docs/SPEC-downstream-ownership-contract.md`
> (§2 — interface đầy đủ). **Giữ file này vì CODE đang reference** —
> `src/modules/account/clients/customer-profile-live.client.ts` trỏ tới đây cho DTO contract; xóa =
> dead-link. Comment đã repoint sang SPEC-downstream; hết reader thì remove được. Không hard-delete
> (repoint-then-remove, cùng kỷ luật cột `users.customerId`).
