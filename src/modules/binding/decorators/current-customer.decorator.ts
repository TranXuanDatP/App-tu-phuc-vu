 ...
 * binding (decrypted per-request, ciphertext-only in cache). On a @RequiresBinding() route
 ...
 *
 * Reaching this decorator without `request.customer` = the route used @CustomerId() without
 * @RequiresBinding() (misconfiguration) → throw loudly (fail fast, not silently fall back).
 */
...
export const CustomerId = createParamDecorator((_data: unknown, ctx: ExecutionContext) => {
  ...
  if (!customer?.customerId) {
    throw new Error(
      '@CustomerId() requires a @RequiresBinding() route (no bound customer resolved)',
    );
  }
